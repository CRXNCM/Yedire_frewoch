...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $update_query = "UPDATE communities SET name = ?, region = ?, description = ? WHERE id = ?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("sssi", $name, $region, $description, $id);
        
        if ($stmt->execute()) {
            // Redirect to manage communities page with success message
            $_SESSION['success_message'] = "Community updated successfully.";
            header("Location: manage_communities.php");
            exit;
        } else {
            $errors[] = "Error updating community: " . $conn->error;
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>Edit Community - Yedire Frewoch</title>
                    ...
                    <div class="card-body">
                        <form method="POST" action="">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($community['name']); ?>" required>
                            </div>
                            <div class="form-group">
                                <label for="region">Region</label>
                                <input type="text" class="form-control" id="region" name="region" value="<?php echo htmlspecialchars($community['region']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="description">Description</label>
                                <textarea class="form-control" id="description" name="description" rows="4"><?php echo htmlspecialchars($community['description']); ?></textarea>
                            </div>
